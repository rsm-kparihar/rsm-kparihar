...
People who visit my website see a landing page with a prompt to join my newsletter. I want to know which version of that prompt gets more people to sign up. One says **“Sign up for our newsletter here!”** and the other says **“Stay up to date by signing up!”**. Even though the two messages are similar, little wording changes like this can influence what people do, so it makes sense to test them.

To do that, I run an A/B test where visitors are randomly split into two groups. One group sees CTA A, and the other sees CTA B. Then I compare the sign-up rates between the two groups. Since the visitors are assigned randomly, any meaningful difference in results can be treated as the effect of the wording itself.

## The A/B Test as a Statistical Problem

Each visitor has two possible outcomes: they either sign up for the newsletter or they do not. Because of that, I can think of each visit as a simple yes-or-no outcome. In statistics, this kind of outcome is often modeled with a Bernoulli random variable. I use $\pi$ to represent the probability that a visitor signs up. Since the wording of the CTA might affect that decision, I allow the sign-up probability to be different for each group. Visitors who see CTA A have a sign-up probability of $\pi_A$, while those who see CTA B have a sign-up probability of $\pi_B$.

The main thing I want to measure is the treatment effect,
...
The Law of Large Numbers says that as the sample gets bigger, the sample average gets closer to the true population average. Here, that matters because my estimator, $\hat\theta = \bar{X}_A - \bar{X}_B$, is based on sample averages. So as I observe more visitors, I expect the estimated difference in sign-up rates to settle closer to the true difference of 0.04.
To show this, I calculate the difference between the outcomes in groups A and B for each observation, then track the cumulative average of those differences over time.
...
plt.plot(np.arange(1, n + 1), running_mean, color="hotpink", label="cumulative (A - B)")
plt.axhline(0.04, linestyle="--", color="deeppink", label="true difference = 0.04")
...
from IPython.display import HTML, display
display(HTML(se_df.to_html(index=False)))

# quick diagnostic plot for the bootstrap distribution
plt.figure(figsize=(8, 4))
sns.histplot(boot_thetas, bins=30, color="lightcoral", edgecolor="firebrick")
plt.axvline(hat_theta, color="black", linestyle="--", label=f"observed = {hat_theta:.3f}")
...
for ax, m in zip(axes, sample_sizes):
    sns.histplot(theta_hats[m], bins=30, color="pink", edgecolor="deeppink", ax=ax, stat="count")
...
from IPython.display import HTML, display
display(HTML(ht_df.to_html(index=False)))
print(f"z = {z_stat:.4f}, two-sided p = {p_value:.4f}")
```
...
from IPython.display import HTML, display
display(HTML(reg_df.to_html(index=False)))
print(f"beta_1 estimate = {beta[1]:.4f}, se = {se_beta[1]:.4f}, t = {t_beta1:.4f}, p = {p_beta1:.4f}")
```
...
from IPython.display import HTML, display
display(HTML(peek_df.to_html(index=False)))
```
## Practical Recommendations for Running A/B Tests

- **Pre-specify your analysis plan:** Decide on sample sizes, primary metrics, and stopping rules before you start collecting data. This protects against inflated false positives from peeking.
- **Choose an appropriate sample size:** Small effects (like a 2-4 percentage point lift) require reasonably large samples to detect reliably; use a power calculation in production.
- **Prefer confidence intervals to single p-values:** Intervals communicate both effect size and uncertainty.
- **Monitor secondary metrics and guardrails:** Make sure improvements in sign-ups do not come at the cost of engagement, conversion quality, or other business goals.
- **Register and document:** Keep a short experiment log with start/end dates, randomization method, and any deviations.

These practices help translate correct formulas into robust, operational decisions when running experiments in production.
